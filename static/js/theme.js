...
(function ($) {
	...
	// Overlay menu 
	// ==================================================
	// بستن منو هنگام کلیک روی لینکهای منو
	$('.tt-ol-menu-list a').not('.tt-ol-menu-social a, .tt-ol-submenu-caret-wrap').on('click', function (e) {
		// جلوگیری از رفتار پیشفرض اگر لینک هش (#) دارد
		if ($(this).attr('href') === '#' || $(this).hasClass('tt-ol-submenu-link')) {
			e.preventDefault();
		}

		// اگر منو باز است
		if ($('body').hasClass('tt-ol-menu-open')) {
			// اجرای انیمیشنهای بستن منو مطابق منطق اصلی
			...
			if ($(".tt-ol-menu-social").length) {
				tl_olMenuOut.to(".tt-ol-menu-social > li", {
					duration: 0.4,
					y: -80,
					autoAlpha: 0,
					stagger: 0.05,
				...
				tl_olMenuOut.to(".tt-ol-menu-social", {
					duration: 0.4,
					autoAlpha: 0,
					clearProps: "all"
				}, "-=0.4");
			...
			tl_olMenuOut.to(".tt-overlay-menu", {
				duration: 0.4,
				autoAlpha: 0,
				clearProps: "all"
			}, "+=0.2");

			// حذف کلاسهای مربوط به منوی باز
			$('body').removeClass('tt-ol-menu-open tt-ol-menu-active');
			$('html').removeClass('tt-no-scroll');

			// بازنشانی وضعیت منو پس از انیمیشن
			setTimeout(function () {
				$(".tt-ol-submenu").slideUp(350);
				$(".tt-ol-submenu-trigger").removeClass("tt-ol-submenu-open");

				// بازگرداندن هدر به جایگاه اصلی اگر اسکرول نرم فعال است
				if ($("body").hasClass("tt-smooth-scroll") && !$("#tt-header").hasClass("tt-header-fixed")) {
					$("#content-wrap").before($("#tt-header"));
				}

				// بازگرداندن منوی اجتماعی به جایگاه اصلی
				if ($(".tt-ol-menu-social").length) {
	...
	if ($("body").hasClass("tt-light-style-on")) {
		$("head").append('<link id="tt-light-style" rel="stylesheet" href="static/css/light-style.css">');
	} else {
